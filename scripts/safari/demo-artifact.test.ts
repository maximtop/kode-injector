...
test.each<[string, (files: ResourceFiles) => void, RegExp]>([
    ['a missing CSS file', (files) => {
        delete files[DEMO_RESOURCE_PATHS.css];
    }, /Missing Safari built-in demo artifact: demo CSS/u],
    ['JavaScript without the marker', (files) => {
        files[DEMO_RESOURCE_PATHS.javascript] = 'document.title = "x";';
    }, /lacks its Kode Injector marker/u],
    ['JavaScript with a syntax error', (files) => {
        files[DEMO_RESOURCE_PATHS.javascript] = 'const id = "kode-injector-demo"; if (';
    }, /JavaScript does not parse/u],
    ['CSS with a syntax error', (files) => {
        files[DEMO_RESOURCE_PATHS.css] = ':root { --kode-injector-demo-css: applied; ';
    }, /CSS does not parse/u],
    ['a background bundle without the demo', (files) => {
        files['background.js'] = 'noop();';
    }, /does not load the built-in demo sources/u],
