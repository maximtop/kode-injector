...
Start the workflow for a published release tag from **Actions → Deploy Apple
App Store → Run workflow**. Publishing the GitHub Release by itself deploys
nothing. Protect the environment with required reviewers if every upload
should need explicit approval.
...
1. Run **Actions → Start release** with the next version (for example
   ...
   drafts the GitHub Release. Publish it, then run **Deploy Apple App Store**
   with that tag. Run the Chrome, Edge, and Firefox deployment workflows
   separately if and when those stores should receive the release; they use
   the same source with the Safari demo compiled out. App Store Connect
   receives a new workflow-derived build number (for example `1.57.1`) of the
   new version.
